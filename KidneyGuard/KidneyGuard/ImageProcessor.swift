...
class ImageProcessor {
    ...
    func wavelength_statistics() -> ([Double], [Double], Int, Double) {
        let hist_n = 200
        var hist_x = [Double](repeating: 0, count: hist_n)
        var hist_v = [Double](repeating: 0, count: hist_n)
        histogram(wavelengths, wavelengths.count, &hist_x, &hist_v, hist_n)
    ...
    func extract_peaks(image: CGImage) -> ([Double], [Int], [Double])! {
        guard let wave = extract_wave(image: image) else { return nil }
    ...
    func extract_wave(image: CGImage) -> [Double]? {
        ...
        guard let context = CGContext(data: pixels_u, width: 1, height: orig_len, bitsPerComponent: 8, bytesPerRow: 1, space: cs, bitmapInfo: 0) else { return nil }
    ...
    func convert(image: CGImage) -> UnsafeMutablePointer<UInt8>? {
        ...
        guard let context = CGContext(data: pixels, width: length, height: length, bitsPerComponent: 8, bytesPerRow: length, space: cs, bitmapInfo: 0) else { return nil }
        context.interpolationQuality = .high
